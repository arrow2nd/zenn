...
:::message
`go install github.com/arrow2nd/nekome/v2@latest` でもインストールできますが、コンシューマーキーが内蔵されていないため、そのままでは使うことができません。
...
例えば、ショートカットのヘルプページは `?` を入力するか、`:docs keybindings` で開くことができます。
...
`~/.config/nekome/preferences.toml` を編集することで設定をカスタマイズすることができます。
