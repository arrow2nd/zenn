...
:::message
この記事は Qiita に投稿した記事に大幅な加筆・修正を施したものです
:::
...
「アプリ開いて、検索窓タップして、保存した検索条件選んで...ってするのめんどくさすぎ」
「RSS リーダは頻繁にチェックするので、ここで見られれば最高なのでは？？」

という気持ちになった結果、

**『Twitter の検索結果を RSS フィードにして、RSS リーダで読めるようにしたい！』**
...
今回使用する Twitter API v2 の `/tweets/search/recent` エンドポイントは [Tweet cap](https://developer.twitter.com/en/docs/twitter-api/tweet-caps) の対象になっており、1 ヶ月に取得できるツイート数が制限されています。

ですので、定期的に検索処理を走らせて生成した XML ファイルをキャッシュしておき、アクセス時にはキャッシュしたものを返すような仕組みにしました。

検索ワードの保存にはお馴染みスプレッドシートを、XML ファイルのキャッシュには GAS の[Cache Service](https://developers.google.com/apps-script/reference/cache/cache-service) を利用しています。
...
`Created new Google Sheet: ` の後に続く URL にアクセスしてスプレッドシートを開き、以下のような形で検索したいワードを列挙します。

Twitter の性質上、ミュート機能は必須（のはず）なので、そのためのエリアも用意しておきます。
...
`fetch()` はツイートを検索して XML ファイルを生成、キャッシュを更新を行う関数です。
これをトリガーで定期実行させることで、継続的なエゴサを可能にします。
...
GAS のトリガー機能を使って、定期的にキャッシュを更新するようにします。
...
普段使っている RSS リーダアプリ [feeder](https://f-droid.org/packages/com.nononsenseapps.feeder/) に登録してみます。
先ほどの URL を入力して検索。
...
いつもの情報収集のついでに、シームレスに担当アイドルのエゴサができるようになったので、ちょっぴり QOL が上がった気がします。
また、自身のエゴサや特定のハッシュタグを購読するのにも便利です。~~（これが正しい使い方な気もする）~~

開発の面では、当初 TypeScript で書いていたところ gs ファイルにうまく変換されず少しつまづいたのですが、後から [制限](https://github.com/google/clasp/blob/master/docs/typescript.md) があることを知りました。
