...
JSON から記事ページを自動生成するようにすることで、以前は必要だった記事ページ毎に JS ファイルを作る作業を削減しました。

また、`arrow2nd.com` のドメインを取得したので設定したところ、Let’s Encrypt のレート制限に引っかかって SSL 証明書がなかなか発行されなかったりもしました（変な操作をした自分が悪い）
...
ゆるい雰囲気のサイトにしたかったので、デザインを一新し、アニメーションも実装しました。 画面遷移のアニメーションについては Qiita に記事を書いています。
...
| 制作時期     | 使用技術                         |
| -------- | ---------------------------- |
| 2021年10月 | Deno, HTML, CSS, Deno Deploy |
