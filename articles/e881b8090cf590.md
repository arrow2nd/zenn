...
Webメディア「[オモコロ](https://omocoro.jp/)」で更新されているWebラジオの非公式ポッドキャストフィードです。

Google Podcastsで配信されているものが、番組毎に別れておらず使いにくかったので作りました。

GitHub Actionsにより1日に2回、オモコロの新着記事RSSフィードを取得・解析し、JSONとポッドキャストフィードを自動生成しています。
...
アイドルマスタシリーズのCDのアートワークを検索できるWebアプリ・APIです。
...
初めはHerokuにデプロイしていましたが、スリープ状態からの復帰に時間がかかるので後にVercelへ移行しました。
