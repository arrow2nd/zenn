...
このひとつの節目を迎えるにあたって、学生時代に作ったものを一気に振り返っていこうと思います。
...
思考ルーチンを積んだ、COMがイカサマしないオーソドックスな4人打ち麻雀ゲームです。
...
ゲーム「アイドルマスタシャイニーカラーズ」内の衣装説明文を検索・共有できるWebアプリケーションです。
...
アイドルマスタシリーズのアイドル・ユニットのイメージカラーを検索できるWebアプリケーションです。
...
`arrow2nd.com` のドメインを取得して設定する際に、変な操作をしてしまいLet’s Encryptのレート制限に引っかかってSSL証明書がなかなか発行されず困ったりもしました。
...
Webメディア「[オモコロ](https://omocoro.jp/)」で更新されているWebラジオの非公式 ポッドキャスト フィードです。

Googleポッドキャストで配信されているものが、番組毎に別れておらず使いにくかったので作りました。

GitHub Actionsにより1日に2回、オモコロの新着記事RSSフィードを取得・解析し、JSONと ポッドキャスト フィードを自動生成しています。
...
アイドルマスタシャイニーカラーズに登場するアイドル、杜野凛世さんのお誕生日までの秒数をカウントダウンする ~~狂気の~~ サイトです。
...
アイドルマスタシャイニーカラーズの公式Twitterに投稿される、Web4コマ漫画を集めたJSONデータです。
...
アイドルマスタシリーズのCDのアートワークを検索できるWebアプリケーション・APIです。

CDを取り込んだ際にアートワークが自動取得されなくて不便に思っていたのと、[SolidJS](https://www.solidjs.com/) が話題になっていて触ってみたかったので作りました。
...
アイドルマスタシリーズの全アイドルのお誕生日をカウントダウンできるサイトです。
...
アイドルマスタの非公式オープンデータセット [im@sparql](https://sparql.crssnky.xyz/imas/) の収録データ数を見ることができるサイトです。
...
アイドルマスタシリーズのアイドルのプロフィール情報を検索できるLINE Botです。

始めたてのころ、なかなかアイドルの顔と名前が一致しなかったので作りました。
作ってきたものの中で一番自分が長く使っているものでもあります。

初めはHerokuにデプロイしていましたが、スリープ状態からの復帰になかなかの時間がかかるので後にVercelへ移行しました。
...
トゲトゲのアスキーアート？を作ることができるLINE Botです。

Web上のジェネレータからLINEにコピー&ペーストするのが面倒に感じたので作りました。
...
スプレッドシートに書いておいたWordを定期検索してSlackに投げるBotです。

Slackが頻繁に鳴るのが心臓に良くなかったので、すぐ使わなくなりました。
...
スプレッドシートに書いておいたWordをTwitterで定期検索して、RSSフィードにするやつです。
...
現在地のこれからの気圧変化が分かるAndroidアプリケーションです。

ビジュアルプログラミングでアプリケーション開発ができるプラットフォーム「[Kodular](https://www.kodular.io/)」を触ってみたくて作りました。
