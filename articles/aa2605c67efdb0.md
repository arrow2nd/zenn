...
ファイルタイプに合わせて、いい感じにコメントアウトしてくれます。
...
補完候補を出力する `source` とその順位付けを行う `filter` とが分離されており、拡張しやすいのが特徴かなと思います。
...
OS 側の IME と辞書を共有するために、起動時に特定のディレクトリ以下にある辞書を探すようにしています。
...
日付やバージョンをいい感じに増減してくれるほか、Bool 値の切り替えなどいろいろできちゃいます。
...
Vim 内でターミナルをいい感じに扱えるプラグインです。
色々機能があるようですが、あんまり使いこなせていません。
